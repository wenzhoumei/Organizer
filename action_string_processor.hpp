...
enum Extension { EXTENSION_SCRIPT, COLOUR, DEFAULT };
enum ExitCode { OK = 0 };

class ActionStringProcessor {
private:
    inline static std::unordered_map<enum Extension, std::string> program_extensions_ = { { Extension::EXTENSION_SCRIPT, "ext" }, { Extension::COLOUR, "colour" }, { Extension::DEFAULT, "" } };
...
public:
	...
	std::ifstream file(action_string + ".space");
